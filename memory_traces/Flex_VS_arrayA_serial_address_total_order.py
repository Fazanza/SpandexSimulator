...
increment_gpu = int(array_length_A/gpu_cores)
...
for iter in range(iteration_no*2):
    if iter % 2 == 0: # CPU Phase
        mem_type = "ld"
        cpu_index_count = [0 for i in range(cpu_cores)]
        threshold = array_length_A # int(array_length_A*0.9)
        mode = "w" if (iter == 0) else "a"
        all_above_threshold = False
        with open(f"total_order.txt", mode) as fh:
            
            while not all_above_threshold:
                available_indices = [i for i, value in enumerate(cpu_index_count) if value < threshold]
                arbiter_out = random.choice(available_indices)
                current_index = cpu_index_count[arbiter_out]
                address = current_index*4
                fh.write(f"{mem_type} {address} cpu_{arbiter_out}\n")
                current_index += dense_stride
                cpu_index_count[arbiter_out] = current_index
                all_above_threshold = all(value >= array_length_A for value in cpu_index_count)
            
            fh.write(f"Phase {iter} End\n")
            # fh.write(f"Phase {iter*2+1} Barrier {gpu_cores}\n")
    else: # GPU Phase
        equal = True if iter != 1 and access_partition == access_partition_start else False
        if equal:
            break
        mem_type = "st"
        gpu_index_count = [access_partition[i]*increment_gpu for i in range(gpu_cores)]
        gpu_index_count_threshold = [((access_partition[i]+1)*increment_gpu-1) for i in range(gpu_cores)]
        all_above_threshold_gpu = False

        with open(f"total_order.txt", "a") as fh:
            while not all_above_threshold_gpu:
                available_indices_gpu = [i for i, value in enumerate(gpu_index_count) if value < gpu_index_count_threshold[i]]
                print(gpu_index_count)
                print(gpu_index_count_threshold)
                print(available_indices_gpu)
                
                arbiter_out_gpu = random.choice(available_indices_gpu)
                current_index_gpu = gpu_index_count[arbiter_out_gpu]
                print(arbiter_out_gpu)
                print(current_index_gpu)
                print("\n")
                address = current_index_gpu*4
                value = random.randint(1,100)
                fh.write(f"{mem_type} {address} {value} gpu_{arbiter_out_gpu}\n")
                current_index_gpu += sparse_stride
                gpu_index_count[arbiter_out_gpu] = current_index_gpu
                all_above_threshold_gpu = all(value >= gpu_index_count_threshold[i] for i, value in enumerate(gpu_index_count))
            fh.write(f"Phase {iter} End\n")
