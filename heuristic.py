...
def owner_benefit(t1: Trace) -> bool:
    ...
    t2: Trace = t1
    t2_prev: Trace = t1
    prevList: List[Trace] = [t1]
    while t2 == Helper.next_conflict(t2):
        if Helper.diff_core(t2_prev, t2) or Helper.sync_possible(t2_prev, t2):
            ...
            y_val = 0
            if Helper.same_core(t2, prevList[-1]):
...
def shared_state_benefit(t1: Trace) -> bool:
    ...
    t2: Trace = t1
    t2_prev: Trace = t1
    while t2 == Helper.next_block_conflict(t2):
        if Helper.diff_core(t2_prev, t2) or Helper.sync_possible(t2_prev, t2):
